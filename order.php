...
<body>
    ...
    $sql = "select o_id, date_order, sum(amount), sum(total), order_status from orders WHERE username = '$username' Group by o_id, date_order" or die("Error:");
    // $sql = "select o_id, date_order, sum(amount), sum(total), order_status from orders WHERE username = '$username' Group by o_id, date_order" or die("Error:" . mysqli_error());
    ...
    <table width="1150" align="center" border="0">
        ...
        ?>
            <tr>
                ...
                <td>
                    <font id="a3"> <?PHP echo $rs['date_order']; ?> </font>
                </td>
                <td>
                    <font id="a3"> <?PHP echo $rs['sum(amount)']; ?> </font>
                </td>
                <td>
                    <font id="a3"> <?PHP echo $rs['sum(total)']; ?> </font>
                </td>
                <td>
                    <font id="a4"> <?PHP echo $rs['order_status']; ?> </font>
                </td>
                <td><a href="viewOrder.php?o_id=<?PHP echo $rs['o_id']; ?>&username=<?PHP echo $username; ?>" id="a2">ดูรายละเอียด</td>
            </tr>
        <?PHP
        }
        mysqli_close($conn);
